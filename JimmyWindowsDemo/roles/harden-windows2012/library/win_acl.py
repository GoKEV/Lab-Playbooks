...
options:
  ...
  state:
    description:
      - Specify whether to add C(present) or remove C(absent) the specified access rule. Use right C(FullControl) and state C(absent) to remove all rights for a given user and type.
    required: no
    choices:
      - present
      - absent
...
# Remove previously added rule for IIS_IUSRS. Removes all rights the IIS_IUSRS user was allowed.
