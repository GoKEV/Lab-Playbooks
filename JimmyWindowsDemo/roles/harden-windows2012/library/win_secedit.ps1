...
Catch {
    ...
    ElseIf ($_.Exception.Message -like "*'$key' cannot be found*"){
        # Key not found. Putting in new key value pair.
    }
    Else {
...
If(-not $check_mode) {
    ...
    Catch [System.Management.Automation.PropertyNotFoundException] {
        ...
        # Fail if value wasn't empty or whitespace.
        If (![String]::IsNullOrWhiteSpace($value)) {
            Fail-Json $result "The key or value you supplied ($key = $value) was not accepted by SecEdit. Ensure it is a valid key value pair and try again. The original key and value were kept in tact"
        }
    }
}
